                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    ...
                                    @if (!empty($product->images))
                                        @foreach (json_decode($product->images, true) as $gimg)
                                            <div class="swiper-slide product-single__image-item">
                                                <img loading="lazy" class="h-auto"
                                                    src="{{ asset('uploads/products/gallery/' . $gimg) }}" width="674"
                                                    height="674" alt="" />
                                                <a data-fancybox="gallery"
                                                    href="{{ asset('uploads/products/gallery/' . $gimg) }}"
                                                    data-bs-toggle="tooltip" data-bs-placement="left" title="Zoom">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <use href="#icon_zoom" />
                                ...
                                <div class="swiper-wrapper">
                                    ...
                                    @foreach (json_decode($product->images, true) as $gimg)
                                        <div class="swiper-slide product-single__image-item"><img loading="lazy"
                                                class="h-auto" src="{{ asset('uploads/products/gallery/' . $gimg) }}"
                    ...
                    <div class="product-single__price">
                        <span class="current-price">
                            @if ($product->sale_price && $product->sale_price < $product->regular_price)
                                <span class="money price price-old">${{ number_format($product->regular_price, 2) }}</span>
                                <span class="money price price-sale">${{ number_format($product->sale_price, 2) }}</span>
                            @else
                                ...
                                <div class="pc__img-wrapper">
                                    <a href="{{ route('shop.product-details', $rproduct->slug) }}">
                                        <img loading="lazy" src="{{ asset('uploads/products/' . $rproduct->image) }}"
                                            ...
                                            class="pc__img">
                                        @foreach (json_decode($rproduct->images) as $gimg)
                                            <img loading="lazy" src="{{ asset('uploads/products/gallery/' . $gimg) }}"
                                                width="330" height="400" alt="Cropped Faux leather Jacket"
                                ...
                                <div class="pc__info position-relative">
                                    ...
                                    <div class="product-card__price d-flex">
                                        @if ($rproduct->sale_price && $rproduct->sale_price < $rproduct->regular_price)
                                            <span
                                                class="money price price-old">${{ number_format($rproduct->regular_price, 2) }}</span>
                                            <span
                                                class="money price price-sale">${{ number_format($rproduct->sale_price, 2) }}</span>
                                        @else
                                            <span
                                                class="money price">${{ number_format($rproduct->regular_price, 2) }}</span>
                                        @endif
                                    </div>
